      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        <div className="relative">
          <div className="inline-block relative">
            {/* Main heart button */}
            <Button onClick={handleHeartClick} className={`w-32 h-32 rounded-full heart-gradient hover:scale-110 transition-magic glow-shadow group ${isPlaying ? 'animate-pulse' : ''}`} size="lg">
              <div className="relative">
                <Heart className={`w-12 h-12 text-white transition-all duration-300 ${isPlaying ? 'scale-125' : 'group-hover:scale-110'}`} fill="currentColor" />
        ...
        <div className="mt-12 flex justify-center">
          <div className="flex items-center space-x-1">
            {[...Array(5)].map((_, i) => <div key={i} className={`w-1 bg-primary rounded-full transition-all duration-300 ${isPlaying ? 'animate-pulse' : ''}`} style={{
