    ...
    <section className="py-20 px-6 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        ...
        <div className="relative inline-block">
          ...
          >
            <div 
              className="w-80 h-80 md:w-96 md:h-96 mx-auto rounded-3xl magic-shadow transition-all duration-700"
              style={{
            ...
            {!isOpened && (
              <Button
                className="absolute inset-0 w-full h-full bg-transparent hover:bg-primary/10 border-2 border-dashed border-primary/30 hover:border-primary/60 rounded-3xl transition-all duration-300"
                variant="ghost"
              >
          ...
          {[...Array(8)].map((_, i) => (
            <div
              key={i}
              className={`absolute w-3 h-3 bg-magic-pink rounded-full sparkle-animation pointer-events-none ${
                isOpened ? 'opacity-100' : 'opacity-60'
              }`}
              style={{
                left: `${Math.random() * 100}%`,
                top: `${Math.random() * 100}%`,
        ...
        <div className="mt-12">
          <p className="font-inter text-lg text-muted-foreground max-w-2xl mx-auto">
            Every pixel of this gift was crafted with love, 
            every animation timed with care, 
